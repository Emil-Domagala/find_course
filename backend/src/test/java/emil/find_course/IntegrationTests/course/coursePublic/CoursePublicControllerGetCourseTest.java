...
public class CoursePublicControllerGetCourseTest extends IntegrationTestBase {
    ...
    private PagingResult<CourseDto> extractContent(MvcResult res) throws Exception {
        JavaType type = objectMapper.getTypeFactory().constructParametricType(PagingResult.class, CourseDto.class);
    ...
    public void coursePublicController_getPublishedCourses_shouldReturnEmptyContentIfNoCourseAfterKeywordFound()
        ...
        MvcResult res = mockMvc.perform(
                MockMvcRequestBuilders.get("/api/v1/public/courses").param("keyword", Instant.now().toString()))
    ...
    public void coursePublicController_getPublishedCourses_shouldSucessfullyReverseOrder(String direction)
        ...
        MvcResult res = mockMvc.perform(
                ...
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.page").value(PaginationRequest.DEFAULT_PAGE + 1))
                .andExpect(MockMvcResultMatchers.jsonPath("$.size").value(PaginationRequest.DEFAULT_SIZE))
                .andReturn();
    ...
    public void oursePublicController_getPublishedCourses_shouldThrow400IfInvalidDirection() throws Exception {
        mockMvc.perform(
                MockMvcRequestBuilders.get("/api/v1/public/courses").param("direction", "invaidDirection"))
    ...
    public void coursePublicController_getPublishedCourses_shouldThrow400IfInvalidCourseCategory() throws Exception {
        mockMvc.perform(
                MockMvcRequestBuilders.get("/api/v1/public/courses").param("category", "invalidCategory"))
                .andExpect(MockMvcResultMatchers.status().isBadRequest()).andReturn();
    }
    ...
    public void coursePublicController_getPublishedCourses_shouldReturnsToDefaultPageSizeSortFieldAndDirection()
        ...
        assertThat(content.getTotalPages()).isEqualTo((int) Math.ceil(count / PaginationRequest.DEFAULT_SIZE) + 1);
